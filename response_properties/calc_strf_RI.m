function [sig, RI, p, RI_thresh, strfcorr, nstrfcorr] = calc_strf_RI(stimulus, spk, trigger, niter1, niter2)
...
for ii = 1:niter1
    ...
    strfcorr(ii,1) = corr(strf1.rfcontra(:), strf2.rfcontra(:));
...
nstrfcorr = zeros(niter2,2);
for ii = 1:niter2
    if mod(ii, 100) == 0
        fprintf('interation %d/niter2', ii)
    end
    newtrain = spiketimes(randperm(length(spiketimes)));
    spk.spiketimes = newtrain(1:n);
    strf1 = calculate_strf_v2(spk, trigger, stimulus, 0.2, 0.05);
    spk.spiketimes = newtrain(n+1:end);
    strf2 = calculate_strf_v2(spk, trigger, stimulus, 0.2, 0.05);
    nstrfcorr(ii,1) = corr(strf1.rfcontra(:), strf2.rfcontra(:));
    %nstrfcorr(ii,2) = corr(strf1.rfipsi(:), strf2.rfipsi(:));
    spk.spiketimes = spiketimes;
end
RI_thresh = quantile(nstrfcorr, 0.99);
p = sum(nstrfcorr >= RI)/niter2;
sig =  RI > RI_thresh;
